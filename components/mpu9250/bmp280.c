...
//i2c_read_bytes    len=2
static esp_err_t read_register16(uint8_t reg, uint16_t *data)
{
    uint8_t d[] = { 0, 0 };
    ESP_ERROR_CHECK(i2c_read_bytes(BMP280_I2C_ADDRESS, reg, d, 2));
...
inline static esp_err_t write_register8(uint8_t addr, uint8_t value)
{
    return i2c_write_byte(BMP280_I2C_ADDRESS, addr, value);
}

static esp_err_t read_calibration_data(bmp280_t *dev)
{
    CHECK(read_register16(0x88, &dev->dig_T1));
    CHECK(read_register16(0x8a, (uint16_t *)&dev->dig_T2));
    CHECK(read_register16(0x8c, (uint16_t *)&dev->dig_T3));
    CHECK(read_register16(0x8e, &dev->dig_P1));
    CHECK(read_register16(0x90, (uint16_t *)&dev->dig_P2));
    CHECK(read_register16(0x92, (uint16_t *)&dev->dig_P3));
    CHECK(read_register16(0x94, (uint16_t *)&dev->dig_P4));
    CHECK(read_register16(0x96, (uint16_t *)&dev->dig_P5));
    CHECK(read_register16(0x98, (uint16_t *)&dev->dig_P6));
    CHECK(read_register16(0x9a, (uint16_t *)&dev->dig_P7));
    CHECK(read_register16(0x9c, (uint16_t *)&dev->dig_P8));
    CHECK(read_register16(0x9e, (uint16_t *)&dev->dig_P9));
...
static esp_err_t read_hum_calibration_data(bmp280_t *dev)
{
    ...
    CHECK(i2c_read_bytes(BMP280_I2C_ADDRESS, 0xa1, &dev->dig_H1, 1));
    CHECK(read_register16(0xe1, (uint16_t *)&dev->dig_H2));
    CHECK(i2c_read_bytes(BMP280_I2C_ADDRESS, 0xe3, &dev->dig_H1, 1));
    CHECK(read_register16(0xe4, &h4));
    CHECK(read_register16(0xe5, &h5));
    CHECK(i2c_read_bytes(BMP280_I2C_ADDRESS, 0xe7, (uint8_t *)&dev->dig_H6, 1));
...
/*
 * obsolete by i2c_master_init in i2c-easy.c
...
{
    i2c_read_bytes(BMP280_I2C_ADDRESS, BMP280_REG_ID, &dev->dig_H1, 1);

    // Soft reset.
    write_register8(BMP280_REG_RESET, BMP280_RESET_VALUE);

    // Wait until finished copying over the NVP data.
    while (1)
    {
        uint8_t status;
        if (!i2c_read_bytes(BMP280_I2C_ADDRESS, BMP280_REG_STATUS, &status, 1) && (status & 1) == 0)
            break;
    }
    ...
    write_register8(BMP280_REG_CONFIG, config);
    ...
    if (dev->id == BME280_CHIP_ID)
    {
        ...
        write_register8(BMP280_REG_CTRL_HUM, ctrl_hum);
    }

    ESP_LOGD(TAG, "Writing ctrl reg=%x", ctrl);
    write_register8(BMP280_REG_CTRL, ctrl);
...
{
    uint8_t ctrl;
    i2c_read_bytes(BMP280_I2C_ADDRESS, BMP280_REG_CTRL, &ctrl, 1);
...
{
    uint8_t status[2];
    i2c_read_bytes(BMP280_I2C_ADDRESS, BMP280_REG_STATUS, status, 2);
...
esp_err_t bmp280_read_fixed(bmp280_t *dev, int32_t *temperature, uint32_t *pressure, uint32_t *humidity)
{
    ...
    i2c_read_bytes(BMP280_I2C_ADDRESS, 0xf7, data, size);
